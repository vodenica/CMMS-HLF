 ...
 */
class ReadPreventiveWorkOrderWorkload extends WorkloadModuleBase {
  constructor() {
    super();
    this.txIndex = 0;
  ...
  async initializeWorkloadModule(
    ...
    for (let i = 0; i < this.roundArguments.assets; i++) {
      ...
      const prev_work_order_id = `${this.workerIndex}_${i}`;

      let prev_work_order_type = "Preventive Work Order";

      let prev_work_order_description = this.description[
        Math.floor(Math.random() * this.description.length)
      ];

      let prev_general_instructions = this.healthandsafety[
      ...
      let prev_work_order_creator = this.creator[Math.floor(Math.random() * this.creator.length)];

      let prev_work_order_tiemstamp = Date.now();

      let prev_condition = "not-good";

      let prev_validation = "not-validated";

      console.log(
        `Worker ${this
          .workerIndex}: Creating preventive work order ${prev_work_order_id}`
      );
      const request = {
        contractId: this.roundArguments.contractId,
        contractFunction: "CreatePreventiveWorkOrder",
        invokerIdentity: "User1",
        contractArguments: [
          prev_work_order_id,
          prev_work_order_type,
          prev_work_order_description,
          prev_general_instructions,
  ...
  async submitTransaction() {
    ...
    const myArgs = {
      contractId: this.roundArguments.contractId,
      contractFunction: "ReadPreventiveWorkOrder",
      invokerIdentity: "User1",
      // Pass the correct value of prev_work_order_id that was created in the initializeWorkloadModule function
...
function createWorkloadModule() {
  return new ReadPreventiveWorkOrderWorkload();
}
