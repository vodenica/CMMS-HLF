 ...
 */
class ReadFailureWorkOrderWorkload extends WorkloadModuleBase {
  constructor() {
    super();
    this.txIndex = 0;
    ...
    this.fault = ["Fault 1", "Fault 2", "Fault 3", "Fault 4", "Fault 5"];
  }
  ...
  async initializeWorkloadModule(
    ...
    for (let i = 0; i < this.roundArguments.assets; i++) {
      ...
      const fail_work_order_id = `${this.workerIndex}_${i}`;

      let fail_work_order_type = "Failure Work Order";

      let fault_id = this.fault[Math.floor(Math.random() * this.fault.length)];

      let fail_work_order_description = this.description[
        Math.floor(Math.random() * this.description.length)
      ];

      let fail_general_instructions = this.healthandsafety[
      ...
      const preventiveMaintenanceProcessSteps = {
      ...
      let fail_work_order_creator = this.creator[Math.floor(Math.random() * this.creator.length)];

      let fail_work_order_tiemstamp = Date.now();

      let fail_condition = "not-good";

      let fail_validation = "not-validated";

      console.log(
        `Worker ${this
          .workerIndex}: Creating failure work order ${fail_work_order_id}`
      );
      const request = {
        contractId: this.roundArguments.contractId,
        contractFunction: "CreateFailureWorkOrder",
        invokerIdentity: "User1",
        contractArguments: [
          fail_work_order_id,
          fail_work_order_type,
          fault_id,
          fail_work_order_description,
          fail_general_instructions,
          JSON.stringify(plannedLabour),
          JSON.stringify(preventiveMaintenanceProcessSteps),
          JSON.stringify(maintenanceParts),
          fail_work_order_creator,
          fail_work_order_tiemstamp,
          fail_condition,
          fail_validation
        ],
        readOnly: false
  ...
  async submitTransaction() {
    ...
    const myArgs = {
      contractId: this.roundArguments.contractId,
      contractFunction: "ReadFailureWorkOrder",
      invokerIdentity: "User1",
      // Pass the correct value of fail_work_order_id that was created in the initializeWorkloadModule function
...
function createWorkloadModule() {
  return new ReadFailureWorkOrderWorkload();
}
